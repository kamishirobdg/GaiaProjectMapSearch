...
**2026-09-19 時点で、8本の CSV と船の上書きはすべて採点済み**（雛形は残っていない。
ユーザー確認済み）。以後は「値を直す」フェーズで、手順は下の「更新の手順」と
「スマホから編集する」のとおり。

**通常版で入れた値を拡張版へ流し込む**には `scripts/copy_base_to_lf.py`
（`/weights` ページの「全コピー」の CLI 版）。両方の版にある行 × 共通の14種族だけを
上書きし、拡張専用の種族と拡張専用の行（25点/3船・拡張タイル）は触らない。
素点が版で違うタイル（`TILE_VP_LF`。いまは TS3 だけ）は比を掛けてから書き込む。
...
**2026-09-13 に全21タイルへ実値が入った**（それまでは全セル 0＝自動フォールバックで、
「6列最大値のコピー」を置いたまま古びる事故を避けていた）。あるタイルの値を消して
0 に戻せば、そのタイルだけまた研究列6つの最大値へ追随する。
...
- `/weights` では上書きの無い船セルを**基準値そのもの**として表示する（2026-09-19）。
  差分に出るのは**基準値と違う船だけ**で、基準値と同じにするために 100% を押す必要は
  ない。上書きのある船セルに 100% を押すと差分に `0` が出て、反映すると上書きが消える
  （＝基準値へのフォールバックに戻る）。
...
評価値は最後に `SETUP_SCORE_DIVISOR`（= 28）で割られ、種族あたり100前後になる
...
- **整合性レビューの提案を画面で読み、採否できる**（2026-09-20）。ヘッダーの「レビュー」で
  全提案の一覧（確信度 高 → 中 → 要判断）。タイルを開くと、そのタイルに関わる提案のカードが
  ルール文の下に出る（見出しを押すと詳細）。根拠のタイル名のボタンで飛び、「← 戻る」で元へ。
  提案が触るセルは枠が点線になる。
  - 「採用」を押すと提案の値が編集に入り、差分に出る。軸なしの行は基準値へ、軸のある行は
    **値そのもの**（`edits.value`。倍率より優先、倍率を押すと外れる）として入る。
  - 「見送り」「未定に戻す」で外れる（手で直して別の値になっていれば残す）。
  - 対象のタイルが無い要判断（列や種族が対象）は一覧の中で開く。「採用」は無く、
    「対応済み」「見送り」を記録するだけ。
  - 採否は別キー `gaia_weight_review` に残り、「全消去」では消えない。差分の末尾に
    `# review P01 = adopt` の記録が付く（反映には使わない）。
  - 提案の元データは `docs/weights-review-<日付>.md` と同名の `.edits.txt`。
    `python scripts/gen_weight_review.py` で `src/gaia/eval/weightReview.ts` を生成し直す
    （自動生成物なので手で編集しない）。
