...
//
// **表そのものは 2026-08-03 に techPositionWeights.ts（自動生成）へ移した。**
// このメモは値を決めるときの参考として残してある。
 ...
 *
 * 2026-08-01: 他カテゴリの基準（SETUP_WEIGHT_BASE、当時5）に対して 1.2倍の 6 にした。
 * 標準技術は9枚が毎ゲーム必ず出て最後まで場に残るので、実プレイでの影響力は
 * どのカテゴリより大きい、というユーザーの評価に合わせたもの（この位置づけは今も同じ）。
 *
 * 2026-08-03: 値を **VP 換算**（中央値12・幅12〜18）へ移したので幅が倍になり、
 * 6 のままだと種族間SD 32.0（占有率41%）まで膨らむ。3 へ下げて移行前と同じ 16.0 に
 * 戻してある。
 *
 * 2026-08-04（e1153fe）: 追加上級を通常の上級の 1/3 にする際、整数比を保つため
 * `SETUP_WEIGHT_BASE` を含む係数体系を一律 ×3 した。この STD_TECH_SCALE も
 * 3→**9** へ追随（比率は不変なので影響力の順は変わらない）。**桁合わせのための
 * 暫定値**（全カテゴリの VP 化が終わったら係数は原則1にし、影響力は VP の幅
 * そのもので決める）。実測は scripts/_probe_category_influence.ts。
 */
