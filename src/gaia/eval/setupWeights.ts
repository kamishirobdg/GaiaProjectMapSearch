 ...
 *
 * 2026-08-04: これは**表示順であって、実測の影響力と一致させる必要はない**
 * （ユーザー判断）。VP 換算でタイルごとの素点を入れたぶん実測は前後する ——
 * たとえば最終得点は全員の順位で大きく差が付くので4〜6位のどこに来てもおかしくない。
 * 順を揃えるために係数を微調整することはしない。
 */
 ...
 * 影響力が狙いの順にぴったり一致する。
 *
 * 2026-08-04（e1153fe）: 追加上級（advExtension）を通常の上級の 1/3 にする際、
 * 整数比を保つため基準・標準技術・上級・ラウンドを一律 ×3 した
 * （3→**9** / 標準技術3→9 / 上級2→6 / ラウンド2→6 / 追加上級は新設で2）。
 * 比率は変えていないので影響力の順は変わらない。以後この最終スケール（40→28、
 * `SETUP_SCORE_DIVISOR`）で桁を合わせている。
 ...
 * 6.6 に戻してある。2026-08-04（e1153fe）に基準の ×3 と合わせて 2→**6** へ
 * 一律スケール（比率は不変）。**桁合わせのための暫定値**（全カテゴリの VP 化が
 * 終わったら係数は原則1にし、影響力は VP の幅そのもので決める）。
 */
 ...
 * 2026-08-04（e1153fe）に基準の ×3 と合わせて 2→**6** へ一律スケール（比率は不変）。
 *
 ...
 * 表の値は既定では通常の上級と同じ（`advancedTechExtensionCell` が6列の最大値へ
 * フォールバック）だが、2026-08-08 に拡張版だけ `vp25`/`shuttle` の専用軸を持てる
 * ようになった（面ごとに未入力なら引き続き6列最大値へフォールバック）。
 * いずれにせよ「取れる見込みの低さ」はこの係数で表す。
 */
...
/**
 * 評価指数の入力範囲。2026-08-01（基準10のとき）に -90..90 として導入して以来、
 * 基準は 5 → 3 → 9（`SETUP_WEIGHT_BASE`）と変わったが、入力範囲は据え置き —
 * 基準に追随させる意図はなく、極端な値まで動かせる余地として固定してある。
 */
