...
#
# 2026-08-06（ユーザー指示）: **全タイルに +4**。上級技術はどの研究列でも
# 進められるのに対し、標準技術はトラック下の6枚がその列でしか進められない。
# その自由度の差が最大4点ぶんある、という評価。下の値は +4 済みで、
# 括弧内が +4 前（＝タイルの効果そのものの見込み）。
TILE_VP = {
    # --- 取得時（即時）: 取得した瞬間の盤面で決まる ---
    "AT04": 18,  # (14) 鉱山×2VP … 鉱山7個
    "AT06": 8,   # (4)  宙域×鉱石1 … 宙域4 → 鉱石4（資源なのでVPは低め）
    "AT08": 10,  # (6)  ガイア惑星×2VP … ガイア3個
    "AT09": 16,  # (12) 交易所×4VP … 交易所3個
    "AT10": 12,  # (8)  宙域×2VP … 宙域4
    "AT12": 15,  # (11) 同盟タイル×5VP … 同盟2〜3枚
    "AT16": 15,  # (11) 首府・学院×6VP … 首府1＋学院0〜1（LF）
    "AT17": 12,  # (8)  深宇宙宙域×4VP … 深宇宙2（LF）
    # --- アクション: 取得後の残りラウンドで**4回起動できる**前提（取得したラウンド中に
    #     もう使えるため。収入タイルは残りラウンド分しか入らない）。
    #     2026-08-08: 「まだ評価が低い」というユーザー指摘を受け、起動回数を3→4回に、
    #     さらに資源レートを知識以外は倍にした（プレイヤー目線の得点価値換算。
    #     鉱石1=2.0 / クレ1=0.8 / 知識1=1.5（据え置き）/ QIC1=3.0 / パワー1=1.0）。
    #     gen_tech_position_table.py の収入タイル（TS1/TS5/TS6）は旧レートのまま
    #     ——このアクション3枚だけの個別調整（2026-08-06 時点では共通だった）。
    "AT03": 32,  # (28.0+4) QIC1＋クレ5 … (1×3.0＋5×0.8)×4回 ＝ 28.0
    "AT07": 28,  # (24.0+4) 鉱石3 … 3×2.0×4回 ＝ 24.0
    "AT13": 22,  # (18.0+4) 知識3 … 3×1.5×4回 ＝ 18.0
    # --- パス時: 取得後およそ3回パスする。建造物は回を追って増える ---
    "AT01": 21,  # (17) 同盟タイル×3VP … 同盟が 1.5→2→2.5 枚と増える×3VP
    "AT05": 20,  # (16) 研究所×3VP … 研究所1.5〜2個×3回
    "AT15": 18,  # (14) 惑星種類×1VP … 種類4→4.5→5×3回
    "AT18": 13,  # (9)  小惑星×2VP … 小惑星1.5個×3回（LF。数が限られる）
    "AT21": 15,  # (11) 深宇宙宙域×2VP … 深宇宙 1.5→2→2.5×3回（LF）
    # --- 誘発（〜するたび）: 取得後の残り3ラウンドで何回起きるか ---
    "AT02": 12,  # (8)  研究を進めるたび+2VP … 取得後4回前後
    "AT11": 9,   # (5)  交易所を建設するたび+3VP … 取得後1.5個（終盤は建て替えが主）
    "AT14": 11,  # (7)  鉱山を建設するたび+3VP … 取得後2個
    "AT19": 11,  # (7)  惑星改造1段階ごと+2VP … 取得後3段階（LF）
    "AT20": 11,  # (7)  QICアクションのたび+4VP … 取得後1.5回（LF。取り合いになる）
}
