...
TEMPLATE_RATIO = 0.25
#
# 2026-08-06（ユーザー指示）に TS1 / TS9 / TS3 を上げた。括弧内が改定前。
TILE_VP = {
    # --- 収入系: 5〜6ラウンドぶん ---
    ...
    "TS8": 10,  # クレ4 × 6R … クレ24
    # --- 即時 ---
    "TS1": 11,  # (3) 鉱石1＋QIC1。**収入系の TS5 に勝る**（2026-08-06）——
                # R1 に取って鉱石1を鉱山に回せば、その鉱山が以降5ラウンド収入を生む。
                # QIC は技術タイル獲得にも入植（ガイア・距離）にも使えて選択の幅が広い。
    "TS2": 5,   # 惑星種類×知識1 … 取得時の種類3個ぶん
    "TS4": 7,   # 7VP そのもの
    # --- アクション・誘発 ---
    "TS9": 12,  # (7) パワー4。**収入系に勝る**（2026-08-06）—— 獲得ラウンドから
                # 効き、しかも自分に都合の良いタイミングで使える。タクロン族・
                # ネヴラ人が最大の効力（列ごとの差は CSV 側に入っている）。
    "TS7": 8,   # ガイア惑星に鉱山＋3VP … 取得後に建てるガイア鉱山2.5個
    "TS3": 12,  # (4) 首府・学院のパワー値4。**同盟1つぶん**（2026-08-06）——
                # きちんと機能させれば同盟を1つ増やせる。拡張版は TILE_VP_LF。
}

# 拡張版だけ素点が違うタイル（2026-08-06）。
TILE_VP_LF = {
    "TS3": 14,  # 金枠同盟と LF船のぶん、同盟1つの価値が通常版より大きい
}
